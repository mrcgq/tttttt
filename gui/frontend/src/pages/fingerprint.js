...
<div class="card">
  <div class="card-header"><h3>🔄 轮换模式</h3></div>
  <div class="card-body">
    <div class="form-row">
...
<div class="fp-card ${fp.name === App.state.selectedFingerprint ? 'selected' : ''}"
  ...
  renderDetail() {
    ...
    return `
<div class="grid-2">
  <div class="form-group"><label>名称</label><div style="font-size:15px;font-weight:600">${fp.name}</div></div>
  <div class="form-group"><label>浏览器</label><div style="font-size:15px;font-weight:600">${fp.browser} ${fp.version}</div></div>
  <div class="form-group"><label>平台</label><div style="font-size:15px;font-weight:600">${fp.platform}</div></div>
  <div class="form-group"><label>标签</label><div style="font-size:15px;font-weight:600">${fp.tags.length ? fp.tags.join(', ') : '无'}</div></div>
</div>`;
  },
  ...
  async testFp() {
    ...
    if (API.connected) {
      try {
        ...
        if (output) output.textContent = JSON.stringify({ error: e.toString(), profile: App.state.selectedFingerprint }, null, 2);
      }
    } else {
      if (output) output.textContent = JSON.stringify({
        profile: App.state.selectedFingerprint,
        status: '离线模式 — 请连接引擎 API 后测试',
