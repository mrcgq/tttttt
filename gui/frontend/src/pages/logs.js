...
<div class="card">
  <div class="card-header">
    ...
    <div class="card-actions">
      ...
      <button class="btn btn-xs btn-secondary" onclick="LogsPage.fetchEngineLog()">📡 获取引擎日志</button>
  ...
  toggleScroll() {
    ...
    App.renderPage('logs');
    App.toast('自动滚动: ' + (this.autoScroll ? '开启' : '关闭'), 'info');
  },
  ...
  async fetchEngineLog() {
    try {
      const lines = await API.getEngineLogLines();
      if (lines && lines.length > 0) {
        lines.forEach(line => {
          const trimmed = line.trim();
          if (trimmed) {
            App.log('debug', trimmed);
          }
        });
        App.toast('获取 ' + lines.length + ' 行引擎日志', 'info');
      } else {
        App.toast('无引擎日志', 'info');
      }
    } catch (e) {
      App.toast('获取引擎日志失败: ' + e, 'error');
    }
  },
