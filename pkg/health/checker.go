...
const (
	...
	StatusDegraded Status = 2 // Slow but functional
	StatusDown     Status = 3
)
...
func (c *Checker) checkOnce(nc *nodeChecker) {
	...
	start := time.Now()
	atomic.AddInt64(&nc.health.TotalChecks, 1)
	...
	if err != nil {
		c.recordFailure(nc, fmt.Sprintf("dial: %v", err))
		return
	}
	result.Conn.Close()
	...
	nc.health.ConsecFailures = 0
	nc.health.LastError = ""
	atomic.AddInt64(&nc.health.TotalSuccesses, 1)
	...
	c.logger.Debug("health check passed",
		zap.String("node", nc.cfg.Name),
		zap.Duration("latency", latency),
		zap.String("status", nc.health.Status.String()),
	)
}
...
func (c *Checker) recordFailure(nc *nodeChecker, errMsg string) {
	...
	atomic.AddInt64(&nc.health.TotalFailures, 1)
	...
	c.logger.Warn("health check failed",
		zap.String("node", nc.cfg.Name),
		zap.Int32("consecutive_failures", nc.health.ConsecFailures),
...
func (c *Checker) GetAllHealth() []NodeHealth {
	c.mu.RLock()
	defer c.mu.RUnlock()
	result := make([]NodeHealth, 0, len(c.nodes))
	for _, nc := range c.nodes {
...
func (c *Checker) BestNode() string {
	...
	for name, nc := range c.nodes {
		...
		if bestName == "" || nc.health.Latency < bestLatency {
			bestName = name
...
func (c *Checker) ResolveEndpoint(name string) (string, error) {
	...
	nc, ok := c.nodes[name]
	if !ok {
		return "", fmt.Errorf("health: unknown node %q", name)
	}
	if nc.health.Status == StatusDown {
		return "", fmt.Errorf("health: node %q is down (consecutive failures: %d)",
