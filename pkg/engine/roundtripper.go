...
func (t *FingerprintTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	if req.URL.Scheme != go-string">"https" {
		return nil, fmt.Errorf(go-string">"engine: only https is supported, got %s", req.URL.Scheme)
	}
 
	host := req.URL.Host
	profile := t.Selector.Select(host)
	if profile == nil {
		return nil, fmt.Errorf(go-string">"engine: no profile selected for %s", host)
	}
	...
	if req.Header.Get(go-string">"User-Agent") == go-string">"" {
		if req.Header == nil {
			req.Header = make(http.Header)
		}
		req.Header.Set(go-string">"User-Agent", profile.UserAgent)
	}
	...
	if result.NegProto == go-string">"h2" {
...
func (t *FingerprintTransport) roundTripH2(host string, conn net.Conn, profile *fingerprint.BrowserProfile, req *http.Request) (*http.Response, error) {
	...
	if err != nil {
		conn.Close()
		return nil, fmt.Errorf(go-string">"engine: h2 client: %w", err)
	}
